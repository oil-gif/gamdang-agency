...
- [ ] **รัน migration 008** (`supabase/migrations/008_booking_checkin.sql` — เพิ่ม `shoot_bookings.arrived_at` + `talent_id`) — จำเป็นก่อนใช้ปุ่ม "🏁 มาถึงแล้ว" (เช็คชื่อ) และ "➕ เพิ่มเข้าระบบ Talent" ในหน้ารอบถ่าย (ตัว Overview grid โชว์ได้เลยไม่ต้องรอ)
