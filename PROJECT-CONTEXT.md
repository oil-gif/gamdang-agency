...
| `/r/[token]` | **ลูกค้า** — ดู Casting Report (token ใบเดียวกับ `/p`) | เหมือน `/p` · ต้องยอมรับ T&C ที่ `/p` ก่อน |
...
| `app/manifest.ts` + `app/apple-icon.png` + `app/admin/layout.tsx` | ไอคอน/ชื่อตอนกด "เพิ่มไปยังหน้าจอโฮม" · **ห้ามใส่ `start_url` ใน manifest** — จะบังคับให้เปิดหน้าแรกเสมอ แม้ผู้ใช้บันทึกหน้า `/admin` ไว้ (เคยพลาดมาแล้ว 2026-09-18 แก้ 2026-09-19) · ไอคอนต้องมีพื้นทึบ iOS ไม่รองรับพื้นโปร่งใส · `app/admin/layout.tsx` มีไว้ตั้งชื่อ "GAMDANG Admin" ให้ไอคอนหลังบ้านแยกจากเว็บสาธารณะ |
| `app/photo/[...path]/route.ts` | ส่งรูปทั้งเว็บ (แปลงเป็น JPEG ให้ LINE เปิดได้) · **ห้ามเปลี่ยนเป็น redirect ไปโดเมน Supabase** — `CompcardGenerator` วาดรูปจากที่นี่ลง canvas ถ้าต่างโดเมน canvas taint ปุ่มสร้างคอมการ์ดพัง · รูปย่อ `?w=320` เก็บถาวรที่ `_cache/w320/…` ใน storage (ลด CPU ของ Vercel) · **ไม่เก็บขนาดเต็ม** เพราะ +662 MB เกิน 1 GB ของ Supabase ฟรี |
